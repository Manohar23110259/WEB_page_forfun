...
import { Sparkles, User, ShieldCheck, Heart, ArrowDown, RefreshCw } from "lucide-react";
import React, { useRef, useState, useEffect } from "react";
import SignatureCanvas from "react-signature-canvas";
import { createClient } from '@supabase/supabase-js';

const supabaseUrl = 'https://zfzkolnwcwwmjdewynpo.supabase.co';
const supabaseKey = 'sb_publishable_Y0pXelgEzrkXGgo9IBkJlQ_YD6jkmcP';
...
export default function Home() {
  ...
  useEffect(() => {
    // Solve Hydration Error: Only generate stars on the client side
    setMounted(true);
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      alert(`Identity Secured for ${subjectName}.`);
      setSubjectName("");
      sigCanvas.current?.clear();
...
<div className="stars">
  ...
  <div className="orb w-[500px] h-[500px] bg-pink-600/05 bottom-[-10%] right-[-10%]" />
      ...
      {/* SECTION 1: HERO - FULL TITLE DISPLAY */}
      <section className="h-screen flex flex-col items-center justify-center px-4 text-center relative z-10">
        ...
        >
          ...
          <h1 className="text-4xl sm:text-6xl md:text-8xl lg:text-[7.5rem] font-serif font-light leading-[1.1] text-white">
            If Sarcasm was a Girl... it looks like you!!<br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-white to-pink-400 italic font-medium">
        ...
        <motion.div animate={{ y: [0, 10, 0] }} transition={{ repeat: Infinity, duration: 2 }} className="absolute bottom-10 opacity-20 text-white flex flex-col items-center gap-2">
        ...
        >
          ...
          <div className="space-y-6">
            <div className="relative group text-left">
              ...
              <input 
                type="text" placeholder="Subject Name" value={subjectName}
            ...
            <div className="flex items-center gap-4">
              ...
              <button 
                ...
                className="flex-1 py-4 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 rounded-2xl text-[10px] font-black uppercase tracking-[0.4em] text-white hover:shadow-[0_0_40px_rgba(99,102,241,0.4)] transition-all active:scale-95 shadow-xl"
              >
                Verify Signature
              </button>
            </div>
